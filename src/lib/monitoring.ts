...
class MonitoringSystem {
  ...
  recordTimer(name: string, startTime: number, tags?: Record<string, string>) {
  ...
  // 🚨 ALERT METHODS
  triggerAlert(severity: Alert['severity'], message: string, service: string, metadata?: Record<string, any>) {
    const alert: Alert = {
  ...
  recordPerformance(event: Omit<PerformanceEvent, 'timestamp'>) {
...
export const metrics = {
  ...
  apiRequest: (endpoint: string, method: string, statusCode: number, duration: number) => {
    monitoring.incrementCounter('api_requests_total', 1, { endpoint, method, status: statusCode.toString() });
    monitoring.recordTimer('api_request_duration', Date.now() - duration, { endpoint, method });
    
    if (statusCode >= 400) {
      monitoring.incrementCounter('api_errors_total', 1, { endpoint, method, status: statusCode.toString() });
    }
  },

  // Database Metrics
  dbQuery: (operation: string, duration: number, success: boolean) => {
    monitoring.recordTimer('db_query_duration', Date.now() - duration, { operation });
    monitoring.incrementCounter('db_queries_total', 1, { operation, success: success.toString() });
  ...
  // Business Metrics
  videoProcessed: (userId: string, duration: number, creditsUsed: number) => {
    monitoring.incrementCounter('videos_processed_total', 1, { user: userId });
    monitoring.recordGauge('video_processing_duration', duration, { user: userId });
    monitoring.recordGauge('credits_consumed', creditsUsed, { user: userId });
  },

  // User Metrics
  userAction: (action: string, userId: string) => {
    monitoring.incrementCounter('user_actions_total', 1, { action, user: userId });
  }
};

// 🚨 ALERT HELPERS
export const alerts = {
  critical: (message: string, service: string, metadata?: Record<string, any>) => 
    monitoring.triggerAlert('critical', message, service, metadata),
  
  high: (message: string, service: string, metadata?: Record<string, any>) => 
    monitoring.triggerAlert('high', message, service, metadata),
  
  medium: (message: string, service: string, metadata?: Record<string, any>) => 
    monitoring.triggerAlert('medium', message, service, metadata),
  
  low: (message: string, service: string, metadata?: Record<string, any>) => 
    monitoring.triggerAlert('low', message, service, metadata)
};
